...
"""get company info from google maps with lat and lng"""
def get_company_info_with_lat_lng(lat, lng):
    GMAPS_API_KEY = os.getenv("GMAPS_API_KEY")
    gmaps = googlemaps.Client(key=GMAPS_API_KEY)
    place_results = gmaps.places_nearby(location=(lat, lng), radius=100, type='point_of_interest')
...
"""get company info from google maps with name, lat and lng"""
def get_company_info_with_name_lat_lng(company_name, lat, lng):
    GMAPS_API_KEY = os.getenv("GMAPS_API_KEY")
    gmaps = googlemaps.Client(key=GMAPS_API_KEY)
    place_results = gmaps.places(query=f"{company_name}", location=(lat, lng), radius=100, type='point_of_interest')
